...
def fire_payload(filePath, encryptPass):
...
def check_key(c, delayTime, filePath, encryptPass, targetTime):
    try:
        twint.run.Search(c)
    except ValueError:
    ...
    if not tweets:
        if (time.time() >= targetTime): fire_payload(filePath, encryptPass)
        else:
            print("No results, trying again after delay")
            time.sleep(delayTime)
            check_key(c, delayTime, filePath, encryptPass, targetTime)
    else:

...
def insert_twitt(c, league, tweet):
    sql = "INSERT INTO wp_bp_twitter( user_id, component, type, action, content, primary_link, date_recorded, item_id, secondary_item_id, hide_sitewide ) VALUES ( %d, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d );" % (
        1, 'groups', 'twitter_update', '', conn.escape_string(
            tweet['user']['screen_name'] + ':' + tweet['text'].encode("iso-8859-1", "ignore").decode("utf-8",
                                                                                                     "ignore")),
        '', get_twitter_date(tweet['created_at']), league[0], '', 0)
    print sql
    c.execute(sql)
